...
export const registerMonacoThemes = (monaco: typeof import('monaco-editor')): void => {
  ...
  const lightTheme: editor.IStandaloneThemeData = {
    ...
    inherit: true,
    rules: [
      { token: '', foreground: '2A1F15', background: 'FFFDF8' },
      { token: 'string', foreground: '7A3E14' },
      { token: 'number', foreground: '0A6A6A' },
      { token: 'keyword', foreground: '8A4B1D' },
      { token: 'type', foreground: '5B3FA0' },
      { token: 'comment', foreground: '8F7D69' },
    ],
    colors: {
      'editor.background': '#FFFDF8',
      'editor.foreground': '#2A1F15',
      'editorLineNumber.foreground': '#8F7D69',
      'editorLineNumber.activeForeground': '#2A1F15',
  ...
  const darkTheme: editor.IStandaloneThemeData = {
    ...
    inherit: true,
    rules: [
      { token: '', foreground: 'D4D4D4', background: '1F1F1F' },
