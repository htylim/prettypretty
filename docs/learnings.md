...
- Do not let embedded-pane fallback e2e coverage inherit the default persisted fallback agent; detached pane formatting now waits for the configured agent, so split-pane tests must install a fast deterministic fallback agent or they will accidentally time out on real CLI latency.
...
- Do not drive output embedded-content detection from Monaco fold ranges or rendered DOM text; resolve candidates from the source text model and project nested decoded matches back to source spans or `Ctrl+click` behavior will drift once the payload is escaped or nested.
- Do not resolve output context-menu actions from only a pixel-perfect Monaco hit position; preserve the current embedded candidate and fall back to the clicked line range or right-click prettify actions become brittle on escaped inline payloads.
- Do not snapshot output context-menu prettify selections only at secondary-click time; real Monaco/Electron secondary clicks can collapse the selection first, so preserve the last non-empty editor selection and use that for menu actions.
- Do not trust synthetic DOM-dispatched editor events to validate Electron modifier gestures; real macOS `Ctrl+click` also emits a `contextmenu` event with left-button metadata, so phase-gate e2e coverage needs actual Playwright mouse/keyboard input or the output editor will reopen the context menu on top of the highlight path.
- Do not restrict output embedded-content detection to quoted payloads; real logs also embed structured objects/arrays directly as field values, and disabling the context-menu action there makes phase 2 look broken.
- Do not treat unchanged prettifier output as a reason to skip opening a pane; extracted payload inspection is still useful for unsupported formats like GraphQL, and a silent no-op makes `Prettify in Pane` look dead.
- Do not assume Monaco output language inference will pick up extracted GraphQL blocks automatically; register GraphQL explicitly and detect it before falling back to plaintext or `Prettify in Pane` will look unhighlighted even when extraction succeeds.
- Do not infer embedded prettify targets from right-click position when the product workflow is explicit selection; use the Monaco selection as the source of truth, then unwrap host delimiters and decode escapes before formatting.
...
- Do not implement `Prettify & Replace` as an output-only rewrite; replace the stored input text and rerun the normal input->output prettifier flow so pane reset, fallback behavior, and downstream state stay consistent.
