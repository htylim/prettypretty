...
- `src/renderer/output/inlineFoldControls.ts` owns output-only inline fold control widgets. It renders Monaco content widgets on visible fold-start lines, refreshes them from scroll/layout/model/language/hidden-area changes, swaps the single inline button between self-toggle and direct-child actions while literal `Ctrl` is held, and prevents inline-control clicks from falling through to pane split selection.
