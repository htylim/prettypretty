...
- `src/renderer/output/inlineFoldControls.ts` owns output-only inline fold control widgets. It renders Monaco content widgets on visible fold-start lines, refreshes them from scroll/layout/model/language/hidden-area changes, exposes separate self-toggle and direct-child buttons per fold start, and prevents inline-control clicks from falling through to pane split selection.
