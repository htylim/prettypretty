...
- `src/renderer/output/inlineFoldControls.ts` owns output-only inline fold control widgets. It renders Monaco content widgets on visible fold-start lines, refreshes them from scroll/layout/model/language/hidden-area changes, keeps self-toggle and direct-child actions separate in the widget state, and handles literal-`Ctrl` inline-control affordances without letting those clicks fall through to pane split selection.
