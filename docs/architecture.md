...
- `src/renderer/app/outputPaneDomain.ts` owns pure derived-pane chain mutations, generalized pane-content descriptors (shared-source range views plus independent text), explicit descendant invalidation, viewport-step math, and active-pane normalization for output split behavior.
...
5. Startup creates one `1600x1050` document window; later document windows can be created from the File menu or renderer IPC without reusing renderer state from an existing window, and new document windows cascade from the originating document window bounds captured at command dispatch with a bounded offset inside the active display work area.
...
- Output pane state keeps content and viewport concerns separate: the ordered derived-pane chain decides which panes exist, and `leftVisiblePaneIndex` decides which adjacent pair is framed in the strip.
- The pane-strip platform is broader than the current product workflow: the current UI keeps the output editor root-only, but the chain model still supports shared-source range panes and independent-text panes for future main-process-backed flows.
...
- Any future output-pane prettify workflow must call the main prettifier service through IPC. Renderer ownership is limited to pane layout, focus, viewport, and editor presentation state.
